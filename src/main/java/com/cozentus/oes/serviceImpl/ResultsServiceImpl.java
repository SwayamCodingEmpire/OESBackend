...
public class ResultsServiceImpl implements ResultsService {
    ...
    public List<SectionSummaryDTO> getSummaryByExamCode(String examCode, Integer id) {
        Exam exam = examRepo.findByCode(examCode)
            .orElseThrow(() -> new ResourceNotFoundException("Exam not found: " + examCode));

        return resultsRepo.findByExamStudent_Exam_CodeAndExamStudent_Student_Id(examCode, null).stream()
            .map(r -> {
                var sec = r.getExamSection();
                return new SectionSummaryDTO(
                    sec.getSection().getName(),    // Topic name as the “sectionTitle”
                    sec.getTotalMarks(),
                    r.getMarksScored()
                );
            })
